...
public interface TelemetrySampleRepository extends JpaRepository<TelemetryEntity, UUID> {
    ...
    Optional<TelemetryEntity> findTopByDevice_DeviceIdOrderByRecordedAtDesc(String deviceId);

    @Query("SELECT t FROM TelemetryEntity t WHERE t.device.deviceId = :deviceId AND t.recordedAt >= :since AND t.recordedAt <= :end ORDER BY t.recordedAt ASC")
